## 基于华为UDX710方案的5G通行壳的改装模块固件/教程：
项目基于STM32F1系列单片机实现

项目电路开源地址：[立创开源平台]https://oshwhub.com/unli/5g-ke-chao-ji-you-hua-si-you

## 特别说明

本项目仅发布编译好的二进制固件文件，不提供任何源代码。

许可条款：
1. 允许个人用户下载、安装和使用本固件，仅限非商业用途
2. 不允许修改、反编译、逆向工程本固件的任何部分
3. 不允许二次分发、复制、传播本固件的任何部分
4. 不允许将本固件用于任何商业目的，包括但不限于出售、出租、捆绑销售
5. 本固件按"原样"提供，不提供任何明示或暗示的保证，作者不对使用本固件造成的任何损失负责

违反以上条款的，作者保留追究法律责任的权利。


## 免责声明
本项目提供的固件仅供参考和学习使用，作者不对因使用本固件而导致的任何损失或损害负责。使用者应自行承担使用本固件的风险，并确保在合法和安全的环境下使用。

**注意：本项目不强制付费，不愿付费的可以直接使用V1.X版本的固件**



**以下教程过程中，有任何问题可以付费咨询，链接同固件付费链接：**
【闲鱼】https://m.tb.cn/h.R1uWIX7?tk=Umjd5s9Vpv1 CZ005 「我在闲鱼发布了【特殊专用链接】」

## 下单说明
1.1、PCB下单——选择导出Gerber或者直接下单

<img src="image/下单1.png" width="600">

1.2、点“否”

<img src="image/下单2.png" width="600">

1.3、点“否，继续下单”

<img src="image/下单3.png" width="600">

1.4、下单参数：板厚1mm,选择3313层压，选择+/-20%阻抗，其他参数按免费选（如果不会那么你不适合这个项目）

<img src="image/参数1.png" width="600">
<img src="image/参数2.png" width="600">

1.5、备注好：“不做半孔，按文件做”

<img src="image/下单4.png" width="600">

2.1、FPC下单——前面步骤一样，全部按照免费选

2.2、此处注意选择“立创EDA设计”，“有金手指”，“0.3”

<img src="image/下单5.png" width="600">

2.3、备注好：“按文件做”

<img src="image/下单6.png" width="600">

下单部分结束


## 组装教程
### 第一部分 接口焊接
必须按照顺序焊接，先焊接24pin排座，再焊接TYPEC接口，typec接口后侧的盖板可以打开，打开后可以通过电烙铁微调焊点。

检查标准：
1、typec接口焊点无连锡
2、24pin排座接点焊接牢固，用镊子一个一个摇晃焊点不松动

<img src="image/接口.png" width="600">

### 第二部分 基础供电组装
依次焊接这三处供电区域

<img src="image/供电.png" width="600">

焊接完成后接上typec，测量以下三个点，依次是3.3V、5V、7.8V、3.3V

<img src="image/供电2.png" width="600">

### 第三部分 CH334+CH340组装及校验
1.1、先焊接此处CH334和两颗电容

<img src="image/CH334F.png" width="600">

1.2、焊接完成后，将typec插到电脑上，打开电脑上的设备管理器，查看“通用串行总线控制器”下是否有出现“通用USB集线器”

<img src="image/CH334F2.png" width="600">

2.1、焊接此处两颗CH340芯片和电容

<img src="image/CH340.png" width="600">

2.2、焊接完成后，将typec插到电脑上，打开电脑上的设备管理器，查看“端口（COM和LPT）”下是否有出现“USB-SERIAL CH340和USB-SERIAL CH340K”

<img src="image/CH3402.png" width="600">

### 第四部分 固件烧录
1、请自行焊接好除7P调试接口以外的所有电路（即下图白框，绿框的磁吸接口也可以先不焊接）

<img src="image/固件烧录.png" width="600">

2、焊接完成后，将typec插到电脑上，打开“固件烧录工具”，选择对应的CH340K对应端口，选择系统固件，选择“DTR为低电平，RTS高电平”

<img src="image/下载.png" width="600">

3、点击“开始编程”，右侧应当出现“IF”，代表正常，等待进度条走完。（如有报错，请检查你前面的焊接的单片机和其他电路部分是否短路或者虚焊）

<img src="image/下载2.png" width="600">

4、下载完成后重新插拔typec，屏幕正常点亮的话，那么恭喜你，成功了

#### 如果到此步未成功，请检查你前面的步骤或者闲鱼咨询我


## 第五部分 授权教程
1、通过【闲鱼】https://m.tb.cn/h.R1uWIX7?tk=Umjd5s9Vpv1 CZ005 「我在闲鱼发布了【特殊专用链接】」付费9.9后，将屏幕显示UID发给我，我会给你授权码。

2、打开“串口通信工具”,选择对应的CH340K对应端口，选择115200波特率，发送授权码

<img src="image/授权.png" width="600">

3、授权完成后，屏幕将自动跳转至主页(注:未授权将导致USB无法连接至通信壳，但是通信壳在正常运行)

### 第六部分 插件导入
1、请自行刷好3.2.4版本的系统和小陈同学的Web,(注，不要刷入小陈同学的合并版固件，否则会脚本运行异常)

2、打开插件商城，点击脚本

<img src="image/插件1.png" width="600">

3、点击右上角的导入脚本，选择你本地的“开源-脚本”导入

<img src="image/脚本1.png" width="600">
<img src="image/脚本2.png" width="600">

4、导入完成后显示为空，并显示获取脚本列表失败属正常现象

<img src="image/脚本3.png" width="600">

5、回到首页点击导入插件，导入成功后出现“串口工具箱”

<img src="image/插件2.png" width="600">
<img src="image/插件3.png" width="600">

6、点运行打开串口工具箱，打开开机自启动（图片为已打开状态）

<img src="image/插件4.png" width="600">

7、打开后重启设备，等待30秒，你的扩展板上的屏幕应当正常显示数据


QQ群：1075838118